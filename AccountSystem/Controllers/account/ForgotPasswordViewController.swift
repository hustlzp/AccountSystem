...
class ForgotPasswordViewController: AccountBaseViewController {
    ...
    override func loadView() {
        ...
        phoneField = PhoneFieldWithSendCodeButton(placeholder: "注册手机号".localized(), didSend: { [weak self] in
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "忘记密码".localized()
    }
    ...
    @objc private func submit() {
        ...
        User.checkPhoneExist(phone).then { (exist) -> Promise<Void> in
            if !exist {
                throw MyError.custom(errorDescription: "该手机号尚未注册".localized())
            }
            ...
            self.navigationController?.pushViewController(ResetPasswordViewController(phone: phone), animated: true)
        }.catch { (error) in
